...
function initPersonalizationPopup(personalization, personalizationMeta, activeKeys, modifyFlag, editFlag, $target) {
	...
	if (!modifyFlag) {
		$popup.find('.personalize-popup-atc-personalization-btn,.personalize-popup-atc-link')
			.attr('data-sku-id', $target.data('sku-id')).data('sku-id', $target.data('sku-id'))
			.attr('data-label', $target.data('label')).data('label', $target.data('label'))
			.attr('data-price', $target.data('price')).data('label', $target.data('price'))
			.attr('data-cat-name', $target.data('cat-name')).data('label', $target.data('cat-name'));
	} else {
		$popup.find('.personalize-popup-add-btn,.personalize-popup-edit-btn')
			.attr('data-item-index', $target.data('item-index')).data('item-index', $target.data('item-index'))
			.attr('data-cartitemids', $target.data('cartitemids')).data('cartitemids', $target.data('cartitemids'));
	}
...
function getPersonalizationPopupFormData() {
	...
	return personalizationTypesArr.map(function(entry) {
		var type = entry[1];
		var val = $popup.find('#field-popup-personalize-' + type).val();
...
function personalizationPopupListeners() {
	...
	$popup
		...
		.on('click', '.personalize-popup-atc-personalization-btn', function(e) {
			...
			linkAtc($btn, personalizationFormData, function() {
		...
		.on('click', '.personalize-popup-add-btn', function(e) {
			...
			updateCartPersonalization(cartItemIds, personalizationFormData);
		})
		...
		.on('click', '.personalize-popup-atc-link', function(e) {
			...
			linkAtc($btn, undefined, function() {
				$popup.removeClass('visible');
...
function personalizationPopupTrigger(target, modifyFlag, editFlag) {
	...
	var personalizationMeta = $target.data('personalizationMeta');
	var activeKeys = $target.data('activeKeys');

	if (personalization && !$.isEmptyObject(personalization)) {
		initPersonalizationPopup(personalization, personalizationMeta, activeKeys, modifyFlag, editFlag, $target);
...
function modifyPersonalizationPopupTrigger(target, editFlag) {
	...
	if (!personalization) {
		try {
			...
			var data = JSON.parse($('.personalize-data-' + itemIndex).text());

			$target.data({
				personalization: data.personalization,
				personalizationMeta: data.personalizationMeta,
				activeKeys: keys
			});
		} catch (err) {
			console.error(err);
	...
	personalizationPopupTrigger($target, true, editFlag);
}
