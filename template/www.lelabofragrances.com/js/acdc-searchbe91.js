...
function itemRenderer(solrsearch) {
	...
	for (var i = 0; i < response.docs.length; i++) {
		...
		doc.refillView = solrsearch.getSettings().refillView;
...
function getPriceHTML(list_price, price, symbol) {
	var price_html = '';
	if (list_price > price) {
		price_html = '<strike>' + symbol + formatListingPrice(list_price) + '</strike>';
		price_html += symbol + formatListingPrice(price);
	} else {
		price_html = symbol + formatListingPrice(price);
	}
